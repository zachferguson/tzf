...
(function() {
  ...
  for (let item in filterIDs) {
    let html = `<div id="${item}" class="selection-box checked"><p>${
      filterIDs[item]["desc"]
    }</p></div>`;
  ...
  document.getElementById("filter-toggle").addEventListener("click", () => {
    if (
      document.getElementById("container-filter").classList.contains("hidden")
    ) {
  ...
  document.getElementById("invert").addEventListener("click", () => {
    ...
    for (let item in filterIDs) {
      ...
      if (document.getElementById(item).classList.contains("checked")) {
        ...
        document.getElementById(item).classList.contains("unchecked")
      ) {
        document.getElementById(item).classList.remove("unchecked");
        document.getElementById(item).classList.add("checked");
      }
  ...
  function projectPopulator() {
    ...
    for (tag in tagsList) {
      for (project in projectDB) {
        if (
          projectDB[project]["tags"].includes(tagsList[tag]) &&
          !displayed.includes(projectDB[project])
        ) {
          document.getElementById(
            "container-portfolio"
          ).innerHTML += projectDetailsPopulator(
            projectDB[project]["title"],
  ...
  function projectDetailsPopulator(
    title,
    blurb,
    projectLink,
    aboutLink,
